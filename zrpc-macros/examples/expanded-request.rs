...
pub trait Hello: Clone {
    fn hello(
        ...
        Box<
            dyn ::core::future::Future<Output = Result<Response<HelloResponse>, Status>>
                + core::marker::Send
                + '_,
        >,
    >;
            ...
            loop {
                ...
                match query.value() {
                    Some(value) => {
                        ...
                        let encoded_resp = match method.as_str() {
                            "hello" => {
                                let req =
...
#[allow(unused)]
#[derive(Clone, Debug)]
pub struct HelloClient<C = zrpc::zrchannel::ZRClientChannel> {
    ch: C,
    server_uuid: ZenohId,
}
...
impl HelloClient {
    pub fn new(z: async_std::sync::Arc<zenoh::Session>, instance_id: ZenohId) -> HelloClient {
        let new_client = zrpc::zrchannel::ZRClientChannel::new(
            z,
